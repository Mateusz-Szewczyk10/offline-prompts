...
class TwoStagePolicy(BasePolicy):
    ...
    def __post_init__(self):
        ...
        self.n_actions, self.dim_action_embedding = self.action_list.shape
        ...
        **kwargs,
        ...
        if return_action_type == "embedding":
            embedding = torch.zeros(
                (n_samples, n_actions_for_each, self.dim_action_embedding),
                device=self.device,
            )
            for k in range(n_actions_for_each):
                action_ids_ = action[:, k]
                embedding[:, k] = self.action_list[action_ids_]
    ...
    def predict_policy_value(
        ...
        predicted_reward: Optional[torch.Tensor] = None,
        clustering_logging_predicted_reward: Optional[torch.Tensor] = None,
        reward_predictor: Optional[BaseActionRewardModel] = None,
...
class SoftmaxPolicy(BasePolicy):
    ...
    Bases: :class:`src.policy.BasePolicy`

    Imported as: :class:`src.policy.SoftmaxPolicy`
    ...
    base_model: BaseActionPolicyModel or BaseClusterPolicyModel or BaseActionRewardModel, default=None
        Base neural network.
    ...
    base_model: Optional[
        Union[BaseActionPolicyModel, BaseClusterPolicyModel, BaseActionRewardModel]
    ] = None
    beta: float = 0.0
    device: str = "cuda"
    ...
    def __post_init__(self):
        self.dim_action_embedding = self.action_list.shape[1]
        ...
        if self.is_first_stage_policy:
            ...
            self.n_actions = self.action_list.shape[0]

        if self.base_model is not None:
            self.given_reward_model = isinstance(self.base_model, BaseActionRewardModel)
        else:
    ...
    def sample_multiple_actions(
        ...
        **kwargs,
        ...
        cluster_centers: torch.Tensor, shape (n_samples, n_clusters, dim_auxiliary_output), default=None
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                action_id_ = F.gumbel_softmax(
                    x[i].reshape((1, -1)) * self.beta, hard=True
                ).argmax(dim=1)
            ...
            for i in range(n_samples):
                action_id_ = F.gumbel_softmax(
                    x[i][candidate_actions[i]].reshape((1, -1)) * self.beta, hard=True
                ).argmax(dim=1)
                action[i] = candidate_actions[i][action_id_]
    ...
    def sample_action_and_output_prob(
        ...
        **kwargs,
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                logit_ = x[i].reshape((1, -1)) * self.beta
            ...
            for i in range(n_samples):
                logit_ = x[i][candidate_actions[i]].reshape((1, -1)) * self.beta
                action_onehot_ = F.gumbel_softmax(logit_, hard=True)
                log_prob_ = (F.log_softmax(logit_) * action_onehot).sum(dim=1)
                action_id_ = action_onehot_.argmax(dim=1)
    ...
    def calc_action_choice_probability(
        ...
        **kwargs,
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                log_prob_ = F.log_softmax(x[i].reshape((1, -1)) * self.beta)[0]
            ...
            for i in range(n_samples):
                log_prob_ = F.log_softmax(
                    x[i][candidate_actions[i]].reshape((1, -1)) * self.beta
                )[0]
                log_prob[i, candidate_actions[i]] = log_prob_
...
class EpsilonGreedyPolicy(BasePolicy):
    ...
    Imported as: :class:`src.policy.EpsilonGreedyPolicy`
        ...
        **kwargs,
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                ...
                greedy_action_ = x[i][candidate_actions[i]].argmax()
                random_action_ = torch.multinomial(
                    torch.ones((n_candidate_actions_,), device=self.device),
                    num_samples=1,
                )
        ...
        **kwargs,
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                ...
                greedy_flg_ = F.one_hot(
                    x[i][candidate_actions[i]].reshape((1, -1)).argmax(dim=1),
                    num_classes=n_candidate_actions_,
                )[0]
                prob[i, candidate_actions[i]] = (
                    greedy_flg_
        ...
        if candidate_actions is None:
            ...
            for i in range(n_samples):
                ...
                if action_flg_.sum() == 0:
                    ...
                    greedy_flg_ = F.one_hot(
                        x[i].reshape((1, -1)).argmax(dim=1), num_classes=self.n_actions
                    )[0]
                    prob_all_ = (
                        greedy_flg_
                        * (1 - self.epsilon + self.epsilon / n_candidate_actions_)
    ...
    def greedy_action(
        ...
        **kwargs,
        ...
        Return
        -------
        greedy_action: torch.Tensor, shape (n_samples, n_actions) or (n_samples, ) or (n_samples, dim_action)
            Greedily chosen discrete actions for each given user context and query.
...
class UniformRandomPolicy(BasePolicy):
    """Uniform random policy for discrete action.

    Bases: :class:`src.policy.BasePolicy`

    Imported as: :class:`src.policy.UniformRandomPolicy`
