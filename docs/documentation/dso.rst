...
For the detailed notations, please also refer to :doc:`implementations`.
...
* :math:`R^{\pi}(x,\phi(s)) := \int_{s' \in \mathcal{S}} \frac{K(s,s'; \, x, \tau) \pi(s'|x)}{\pi(\phi(s)|x)} R(x,s') ds', \, \forall \pi` (expected reward).
...
In contrast, a small value of :math:`\tau` helps keep the bias small, as the within-neighbor reward shift (i.e., the difference between :math:`R^{\pi_0}(x,\phi(s))` and :math:`R^{\pi_{\theta}}(x,\phi(s))`) becomes small. 
...
Full-LLM experiment
----------
While we omit the detailed experiment setting here, we also conduct experiment using a semi-synthetic benchmark built on the MovieLens-10M dataset :cite:`harper2015movielens`.
