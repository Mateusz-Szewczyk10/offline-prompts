"""Script for generating dataset, regressing rewards, and fitting the marginal density model."""
...
def _process(
    ...
    **kwargs,
    ...
    Path(f"logs/logged_feedback").mkdir(exist_ok=True, parents=True)
    ...
    logged_feedback = generate_and_save_logged_data(
        ...
        save_path=Path(
            ...
            f"{reward_simulator_type}_{n_actions}_{dim_query}_{dim_prompt}_{dim_sentence}_{reward_type}_{reward_std}_{n_samples}_{beta}_{device}_{base_random_state}_{random_state}.pkl"
        ),
    )

    # logging marginal density estimator
    ...
    train_and_save_kernel_marginal_estimator(
        ...
        save_path=Path(
            ...
            f"{reward_simulator_type}_{n_actions}_{dim_query}_{dim_prompt}_{dim_sentence}_{reward_type}_{reward_std}_{n_samples}_{beta}_{tau}_{device}_{base_random_state}_{random_state}",
        ),
        device=device,
    ...
    train_and_save_reward_predictor(
        ...
        save_path=Path(
            ...
            f"{reward_simulator_type}_{n_actions}_{dim_query}_{dim_prompt}_{dim_sentence}_{reward_type}_{reward_std}_{n_samples}_{beta}_{device}_{base_random_state}_{random_state}",
        ),
        device=device,
        random_state=random_state,
        use_wandb=use_wandb,
...
def main(cfg: DictConfig):
    ...
    conf = {
        ...
        "n_epochs": cfg.offline.n_epochs_predictor,
        "n_steps_per_epoch": cfg.offline.n_steps_per_epoch_predictor,
        "lr": cfg.offline.lr_predictor,
        "optimizer": get_optimizer(cfg.offline.optimizer_predictor),
        ...
        "n_random_state": cfg.offline.dataset_n_random_state,
        "start_random_state": cfg.offline.dataset_start_random_state,
